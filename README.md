# Google Maps Place Picker Plus

A Flutter plugin which provides 'Picking Place' using [Google Maps](https://developers.google.com/maps/) widget.

This is an actively maintained fork of [google_maps_place_picker_mb](https://pub.dev/packages/google_maps_place_picker_mb) with bug fixes, deprecation resolutions and broader Flutter compatibility.

To install:

```
flutter pub add google_maps_place_picker_plus
```

The project relies on the following packages:

- Map: Flutter's official [google_maps_flutter](https://pub.dev/packages/google_maps_flutter)
- Current location: [geolocator](https://pub.dev/packages/geolocator)
- Place and Geocoding API: [flutter_google_maps_webservices](https://pub.dev/packages/flutter_google_maps_webservices)
- State management: [provider](https://pub.dev/packages/provider)
- Utilities: [tuple](https://pub.dev/packages/tuple)

## Preview
![](preview.gif)

> The preview shows the pick-area restriction feature: the user can only select addresses inside the highlighted circle. This can be disabled.
...
initialMapType | MapType | MapTypes of google map. Defaults to normal.
...
introModalWidgetBuilder | WidgetBuilder | Specified on below section
...
region | String | The region code, specified as a ccTLD two-character value. This parameter will only influence, not fully restrict, search results. When this parameter is used, the country name is omitted from the resulting formatted_address for results in the specified region.
pickArea | CircleArea | Circle that defines the area in which the address can be picked. Can be colored as preferred.
...
searchForInitialValue | bool | Whether to automatically search for initial value on start
myLocationButtonCooldown | int | Cooldown time in seconds for the 'myLocationButton'. Defaults to 10 seconds. 
forceSearchOnZoomChanged | bool | Whether to allow place search even when the zoom has changed. Defaults to false.
...
onTapBack | VoidCallback? | Called when leaving the picker by pressing the back button.
...
By default the map will show up immediately. You might want to show a customized overlay modal with instructions on first load. Use `introModalWidgetBuilder` for full branding freedom.
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        ElevatedButton(onPressed: close, child: Text("OK")),
                      ],
                    ),
                  ),
                ),
...
---------- | ---- | -----------
context | BuildContext | Flutter's build context value
close | Function | Function to be called to close the modal.
...
[tracker]: https://github.com/chjsoliveira/google_maps_place_picker_mb/issues
